...
from ret_eval_utils import evaluate_retrieval, retrieval_res_save
...
def main(data_name, retriever):
    ...
    data_path = os.path.join(base_dir, f'data/process_data/{data_name}/{data_name}_routed.json')
    emb_path = os.path.join(base_dir, f'data/data_emb/{data_name}/{data_name}_{retriever}.pt')
    save_path = os.path.join(base_dir, f"result/retrieval/{data_name}/{data_name}_{retriever}.json")
    csv_path = os.path.join(base_dir, f"result/retrieval/{data_name}/{data_name}_{retriever}.csv")
    ...
    for sample in tqdm(data):
        ...
        for qa in sample['qa']:
            ...
            routed_ranks = memo.retrieve(qa, sample, routed_ids, routed_embs)
    ...
    retrieval_res_save(csv_path, retrieval_res)
