...
from experiment.data_utils import load_session, memory_route, memory_setup, process_em
...
def main(data_name, batch):
    ...
    save_path = os.path.join(base_dir, f"data/process_data/{data_name}/{data_name}_memory_routed.json")
    os.makedirs(os.path.dirname(save_path), exist_ok=True)

    sess_res = {}
    session_dicts, session_ids = load_session(load_path)
    ...
    with open(save_path, "w", encoding="utf-8") as f:
        json.dump(sess_res, f, ensure_ascii=False, indent=4)


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="route the memory to its type.")
